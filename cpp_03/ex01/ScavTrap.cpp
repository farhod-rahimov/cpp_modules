...
void ScavTrap::rangedAttack(std::string const & target) {
    ...
    {
        std::cout << "Cannot do RANGED ATTACK attack because ScavTrap IS DEAD! <rangedAttack>"<< std::endl;
        return ;
    }
    std::cout << "Babakh...! ScavTrap <" << this->_name << "> attacks <" \
...
void ScavTrap::meleeAttack(std::string const & target) {
    if (this->_hit_points <= 0)
    {
        std::cout << "Cannot do MELEE attack because ScavTrap IS DEAD! <melleAttack>"<< std::endl;
        return ;
    }
    std::cout << "Baauumm...! ScavTrap <" << this->_name << "> attacks <" \
...
void  ScavTrap::takeDamage(unsigned int amount) {
    ...
    if (this->_hit_points <=0)
    {
        std::cout << "ScavTrap IS DEAD! <takeDamage>"<< std::endl;
        return ;
    }
    std::cout << "Aaaaa...!!! " << "<ScavTrap takes " << amount << " damage!>";
    ...
    if (this->_hit_points <= 0)
        std::cout << "ScavTrap DYIED! <takeDamage>"<< std::endl;
};

void  ScavTrap::beRepaired(unsigned int amount) {
    if (this->_hit_points == this->_max_energy_points)
    {
        std::cout << "ScavTrap HAS ALREADY BEEN FULLY REPAIRED! <beRepaired>"<< std::endl;
        return ;
    }
    if (this->_hit_points + amount >= this->_max_energy_points)
        ...
        this->_hit_points += amount;
    std::cout << "Yeahhh boy...!!! <ScavTrap HAS BEEN REPAIRED to " << amount << " HPs!>";
